...
export const CardAPI = ({ itens, setOpenModalComment }: iPropsCard) => {
  ...
  const { setPegaIDCard, setPegaInfoCard, listCommentsRequestAdmin } = useContext(AdminContext);
  ...
  return ( 
    <StyledCardAPI>
      ...
      <div className="divFooterCard">
        <button className="BTNBlue" onClick={() => OpenModalIDVerUser()}>Ver API</button>
      </div>
    </StyledCardAPI>
  );
};
